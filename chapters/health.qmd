...
See [set up chapter](set-up.qmd) for how the variables used here were calculated. First, we calculate the number of shrimp that die on farms from the postlarval stage onward. We do so handling the data in the same way as the Set-up chapter, using the `kde1d` function to generate 100,000 samples.
